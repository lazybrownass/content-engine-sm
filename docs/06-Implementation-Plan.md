...
**Testing:** `@axe-core/playwright` scans on every primary route (login, dashboard, knowledge, topics, posts, schedule, analytics, settings, generate); Lighthouse CLI run against an unauthenticated route; `tests/integration/deletion-cascade.test.ts` proving the FK cascade chain.
**Risks:** a strict CSP or aggressive rate limiting can silently break auth/streaming flows in production even when local dev looks fine — mitigated by verifying against a real `npm run build && npm run start` (not just `npm run dev`) and the full e2e suite, not code review alone.
...
#### Phase 6 (continued, part 2) — Accessibility, Performance, Security & Data Export

**Objectives:** close the real Phase 6 checklist from the top of this section (a11y pass, Lighthouse audit, security review, data export). Recorded here per AGENTS.md §10.1, same convention as the Docker insertion above.

**Accessibility:** global `prefers-reduced-motion` handling (`app/globals.css`); arrow-key roving focus on the hand-rolled tablist in `features/generation/components/generation-workspace.tsx`; `aria-expanded`/`aria-controls` on the provider-setup toggle (`features/publishing/components/provider-setup-form.tsx`); an `app/(auth)/layout.tsx` `<main>` landmark plus a real `<h1>` on `/login` and `/forbidden` (both previously rendered with no landmark and no level-one heading — found by the new axe coverage, not assumed). `@axe-core/playwright` scans now cover every primary route: `/login`, `/knowledge`, `/posts`, `/analytics`, `/settings`, plus the pre-existing `/generate`, `/schedule`, `/topics`/`/posts`/editor coverage. **Not done:** resizing buttons to the design brief's 44×44 touch-target guidance — stricter than WCAG 2.1 AA's actual 24×24 minimum (already met), and a global button-size change was judged too disruptive for the actual compliance bar this phase targets; tracked separately if ever revisited.

**Performance/SEO:** per-route `metadata` on every primary page; `app/robots.ts` disallowing all crawling. Lighthouse (`npx lighthouse`) against `/login`: **Performance 98, Accessibility 100, Best Practices 100, SEO 63** (target was ≥90 for all four). SEO intentionally sits below target: Lighthouse's `is-crawlable` audit penalizes the `Disallow: /` in `robots.txt`, but this app is entirely auth-gated with no public content — deliberately blocking search-engine indexing is the correct choice for a private, single-owner tool, not a regression to fix. Without that one audit, every other SEO signal (viewport, meta description, etc.) passes.

**Security:** `next.config.ts` `headers()` — CSP (production-only; `next dev`'s HMR needs a more permissive policy the same headers() call doesn't apply to dev), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy`. `lib/security/rate-limit.ts` (in-memory, per-instance fixed-window limiter — not distributed; acceptable for this app's single-owner/low-traffic profile) applied to both webhook routes and both cron routes. `lib/security/cron-auth.ts` replacing the cron routes' `!==` secret comparison with `timingSafeEqual`, matching `lib/publishing/signing.ts`'s existing HMAC pattern. RLS policy audit and secret-handling audit: verified already correct (owner-scoped RLS on every table; no secrets in any `NEXT_PUBLIC_*` var) — no code change needed. **Explicitly not built, tracked separately:** an `AuditLog` table with write-through calls in every Server Action (mentioned in `docs/02-TRD.md` §7 but not part of this phase's own checklist above) — a much larger schema/call-site change than this hardening pass scopes to.

**Data export & deletion:** `features/settings/actions.ts`'s `exportAccountData()` + `/settings` page — one JSON export of every owner-scoped model (settings, knowledge items, posts, topics, brand voices, pipeline/AI runs, analytics snapshots, style memory), per `docs/01-PRD.md`'s data-portability requirement. `tests/integration/deletion-cascade.test.ts` proves the FK `ON DELETE CASCADE` chain declared in `prisma/schema.prisma` actually removes every child row (including pgvector `KnowledgeChunk` rows) when a `User` is deleted. **Explicitly not built, tracked separately:** a user-facing "delete my account" flow — not a PRD requirement (only export/portability is), and the cascade test above verifies the schema-level guarantee without needing one.

**Testing:** `tests/e2e/{posts,analytics,settings}.spec.ts` (new), axe additions to `auth-redirect.spec.ts`/`knowledge-search.spec.ts`; `tests/integration/deletion-cascade.test.ts`; full `npm run lint && npm run typecheck && npm run test:unit && npm run test:integration && npm run test:e2e` run clean.
**Risks:** the in-memory rate limiter resets on every deploy/restart and isn't shared across replicas — fine for this app's traffic profile today, but would need a distributed store (e.g. Upstash) if that changes. The CSP's `'unsafe-inline'` on `script-src`/`style-src` (needed for Next.js's non-nonce hydration bootstrap and Tailwind/Base UI inline styles) means it doesn't block inline script injection — it still blocks the more common real-world threat (loading a script or connecting to any origin other than this app and Supabase).
**Definition of Done:** same bar as other phases (§14) — lint/typecheck/tests pass, docs (this section) match the shipped implementation. The Phase 6 checklist at the top of this section is now closed, with the two explicitly-scoped exceptions noted above (`AuditLog`, account deletion) tracked as separate, future work rather than silently done or silently dropped.
