...
**Implementation note (Phase 5, partial):** a backend slice of the Analytics + Learning Loop, the topic-scoring feedback loop, and a read-only analytics dashboard have been built — see `docs/06-Implementation-Plan.md`'s "Phase 5 (partial)" section. Built as specified above: `AnalyticsSnapshot`, `StyleMemoryProfile`, and `StyleMemoryExample` (all three with RLS in the migration). `AnalyticsSnapshot` and `StyleMemoryExample` carry no direct `ownerId`; their RLS policies scope via a join (`→ posts.ownerId` / `→ style_memory_profiles.ownerId`), matching the `schedules`/`publishing_jobs` pattern. The profile is recomputed synchronously on every analytics write (plus an explicit `recomputeStyleMemory()` server action) rather than via a cron — `features/analytics/style-memory.ts` holds the pure aggregation (`MIN_SAMPLE_POSTS = 10` gates the loop while data is thin), and the winning style is injected into the generation and pipeline-draft prompt builders via `buildStyleMemoryBlock` (`features/generation/prompt.ts`). The topic-scoring heuristic update also shipped, but required **no new Prisma model or migration** — `getPillarPerformance` (`features/analytics/queries.ts`) computes per-pillar engagement directly over the existing `Post` ↔ `AnalyticsSnapshot` join, gated on the same sample threshold, and a pure post-processing function (`applyPillarScoreBoost`, `features/analytics/pillar-scoring.ts`) adjusts each LLM-suggested topic's score before `generateTopicSuggestions` persists it — so `Topic.score` (already `Float?` in the schema) can now differ from the raw LLM output. The `/analytics` dashboard is a read-only view (`app/(app)/analytics/`) built with direct Prisma queries — no new schema, no materialized view. Deviations from the target: no `Post.publishedAt` column (the computation filters `status = PUBLISHED` ordered by `updatedAt`). Deferred entirely this phase: the `Feedback` model, the `post_performance_rollup` view and `recent_post_phrases` materialized view + refresh trigger, the manual analytics-entry form UI and Recharts charts specifically, the Playwright analytics adapter, and the `/api/cron/refresh-style-memory` route. The definitions above remain the target shape for full Phase 5.
