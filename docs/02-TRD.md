...
| Reranking retrieved knowledge | **BAAI/bge-reranker-v2-m3** (supersedes the originally-planned `cross-encoder/ms-marco-MiniLM-L-6-v2` — that model is not deployed on any HF Inference Provider as of implementation, verified against HF's own docs; bge-reranker-v2-m3 is the model HF's `text-classification` task docs use as their live example) | Cross-encoder built on bge-m3, Apache 2.0 | Reranking the deduped FTS+vector candidate pool (~20) down to the top ~5 meaningfully improves what actually reaches the Writing stage, at negligible extra latency because it only scores a short list | <1s for top-20 |
