...
import { POST } from "@/app/api/webhooks/make/route";
...
beforeAll(async () => {
  ...
  await prisma.user.create({ data: { id: ownerA, email: `make-callback-${ownerA}@example.com` } });
  provider = await prisma.automationProvider.create({
    data: {
      ownerId: ownerA,
      type: "MAKE",
      label: "make",
      configRef: "https://make.test/hook",
...
function makeRequest(body: string, signature: string | null): NextRequest {
  ...
  return new NextRequest("http://localhost/api/webhooks/make", { method: "POST", body, headers });
}
